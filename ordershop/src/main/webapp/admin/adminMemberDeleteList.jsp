            ...
            <div class="content_wrap">
                ...
                <section id="admin_section">
                   ...
                   <div class="top_menu">
                      ...
                      <!-- search_menu -->
                      <form method="get" action="/adminMemberDeleteList">
               ...
               <c:forEach var="deletemember" items="${deletememberList}" varStatus="status">
                   <tr>
                     <td>${total - (status.index)}</td>
                       <td>${deletemember.USER_NO}</td>
                       <td>${deletemember.USER_ID}</td>  
                       <td>${deletemember.USER_NAME}</td>
                       <td>${fn:substring(deletemember.USER_JOIN,0,10)}</td> 
                       <td>${fn:substring(deletemember.DELETE_DATE,0,10)}</td>
                       <td>${deletemember.DELETE_REASON}</td>
                     ...
                     <!-- 페이징 -->
<div class="div_page">
  <a href="/adminMemberDeleteList?pageIndex=1" class="page_arrow">«</a>
  <a href="/adminMemberDeleteList?pageIndex=${currentPage - 1}" class="page_arrow">‹</a>                    

  <c:forEach var="p" begin="1" end="${totalPage}">
    <c:if test="${p == currentPage}">
      <a href="/adminMemberDeleteList?pageIndex=${p}" class="active">${p}</a>
    </c:if>
    <c:if test="${p != currentPage}">
      <a href="/adminMemberDeleteList?pageIndex=${p}">${p}</a>
    </c:if>
  </c:forEach>

  <a href="/adminMemberDeleteList?pageIndex=${currentPage + 1}" class="page_arrow">›</a>
  <a href="/adminMemberDeleteList?pageIndex=${totalPage}" class="page_arrow">»</a>
</div>
